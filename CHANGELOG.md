...
## [PR #116] feat: Claude widget interactive prompts — Allow/Deny cards, questions, plan approval, loose sandbox
...
### Context
PR #106 removed the Claude widget on the assumption Nish couldn't use it (Claude *desktop app* vs the Code CLI it needs). That was premature — Nish still wants the widget; the "not found" was just a login/setup diagnosis in progress. This reverts #106 in full, bringing the widget, `/api/claude` route, `claudeCli` bridge, shared types, and the `CLAUDE_CODE_OAUTH_TOKEN` credential back exactly as they were.

### Notes
- The widget still needs the Claude Code CLI installed + `claude /login` (bills the Max plan, no API key). The desktop app can't back it.
- The two defensive guards #106 added (DashboardGrid unknown-id skip + layoutStore rehydrate pruning) are reverted with it — back to the pre-#106 state.
