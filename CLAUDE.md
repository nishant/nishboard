...
| Claude | spawns the user-installed **Claude Code CLI** (`claude -p --output-format stream-json --include-partial-messages --verbose`, prompt via stdin, `--resume` for context, cwd `~/.dash`) — bills the claude.ai **Max subscription**, never API keys; `ANTHROPIC_API_KEY` is stripped from the child env. Assistant **text streams via `delta` frames only** (no authoritative-final "message" frame — a tool-using turn has multiple assistant turns that would clobber each other); `parseStreamJsonLine` returns an **array** of events per line. A **Settings → Claude → "Allow tools"** toggle (default off) adds `--permission-mode bypassPermissions` so the CLI can actually write files / run commands / invoke skills — off ⇒ non-interactive `-p` auto-denies Write/Edit/Bash. Tool calls render as inline **chips** (`tool-use`/`tool-result` events → ordered `parts` in `claudeStore`) | none (CLI's own OAuth login; optional CLAUDE_CODE_OAUTH_TOKEN fallback) | SSE stream on demand |
